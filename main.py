...
def add_page(id):
    ...
    return render_template('add_page.html',success=request.args.get('success'), error=request.args.get('error'),validation = request.args.get('validation'), fields = fields, id= id)
...
def add_User_phone_no():
    ...
    qry = "INSERT INTO User_phone_no Values (%s,%s)"%val[:-1]
    print(qry)
    success = True
    error = False
    try:
    ...
    return redirect(url_for('add_page', id='User_phone_no', error=error,success=success, validation = validation))
...
def add_orphanage():
    ...
    return redirect(url_for('add_page', id='orphanage', error=error,success=success, validation = validation))
...
def add_orphanage_stats():
    ...
    return redirect(url_for('add_page', id='orphanage_stats', error=error,success=success, validation = validation))
...
def add_Donor():
    ...
    qry1 = "INSERT INTO Donor Values (%s,%s,%s,%s,%s,%s,%s)"%val
    qry2 = " update donation_camp set Amount_collected = %s + Amount_collected where DC_ID = %s;"%(val[4], val[6])
    print(qry1, qry2)
    success = True
    error = False
    try:
        ...
        error = True
        success = False
        return redirect(url_for('add_page', id='Donor', error=error,success=success, validation = validation))
    mydb.commit()
...
def add_donation_camp():
    ...
    qry = "INSERT INTO donation_camp Values (%s,%s,%s, %s, %s, %s, %s)"%val
    print(qry)
    success = True
    error = False
    try:
    ...
    return redirect(url_for('add_page', id='donation_camp', error=error,success=success, validation = validation))
...
def add_Branch():
    ...
    return redirect(url_for('add_page', id='Branch', error=error,success=success, validation = validation))
...
def update_doctor_details():
    ...
    try:
        ...
        print("update error")
        return render_template('error_page.html', validation = e)
    mydb.commit()
...
def remove_branch():
    if not session.get('login'):
        return redirect( url_for('home') )
    return render_template('/remove_branch.html', validation = '')
...
def del_hostel():
    ...
    try:
        ...
        validation = e
    mydb.commit()
    return render_template( 'remove_user.html' , validation = validation)
...
def del_orphanage():
    ...
    try:
        ...
        validation = e
    mydb.commit()
    return render_template( 'remove_orphanage.html' , validation = validation)
...
def del_donor():
    ...
    try:
        ...
        print("Error in deletion")
    mydb.commit()
    return render_template( 'remove_donor.html' , validation = validation)
...
def del_branch():
    ...
    try:
        ...
        validation = 'error'
    mydb.commit()
    return render_template( 'remove_branch.html' , validation = validation)
...
def del_donation_camp_head():
    ...
    try:
        ...
        print("Error in deletion")
    mydb.commit()
    return render_template( 'remove_donation_camp.html' , validation = validation)
#------------------------------------------------------------------------
...
#----------------------------------------------predict model--------------------------------------------
@app.route("/selectpredict")
def file():
    return render_template("predict.html", title = 'Expense prediction', features = 'strength', modelname = 'Expense prediction', filename = 'Expense_prediction.pkl', message = "", cls= 'invisible')


@app.route("/predict_page", methods= ["GET", "POST"])
def predict_page():
    if request.method == "POST":
        return render_template("predict.html", title = 'Expense prediction', features = ['strength', 'expense'], modelname = 'Expense prediction', filename = 'Expense_prediction.pkl',  message = "", cls= 'invisible')
    
@app.route("/inputpredictfile", methods = ["GET", "POST"])
def isvalidfile():
    if request.method == 'POST':
        f = request.form['file']
        print(f)
        # print('hi')
        fname =f
        if re.search(r'.pkl$',fname):
            title = fname.replace('.pkl', '')
            file1 = open('model_info/'+title+'.txt', 'r')
            content = file1.read()
            print(content)
            templist = content.split('\n')
            modelname = templist[0]
            features = templist[1:]
            return render_template("predict.html", title = title.replace('_',' '), features = features, modelname = modelname, filename = fname, message = "", cls= 'invisible')


@app.route("/predict", methods=["GET", "POST"])
def result():
    # global modelname, features
    if request.method == "POST":
        # attend = request.form['Attendance']
        # mse = request.form['mse']
        # print(attend, mse)
        # endsem = model.predict([[attend, mse]])
        features = request.form['features']
        modelname = request.form['modelname'].replace(' ','_')
        model = pickle.load(open('Expense_prediction.pkl', 'rb'))
        print(features, modelname)
        x = re.findall(r'\'\w+\'', features)
        print(x)
        a = ''
        b = []
        for i in x[:-1]:
            print(i.replace("'", ''))
            a = request.form[i.replace("'", '')]
            b += [float(a)]
        print(b)
        flash("your endsem result may be") # + str(endsem)
        prediction =  model.predict([b])
        message = "your "+ str(x[-1]) +"predicted is ..Rs." + str(prediction[0])
        return render_template("predict.html",title = modelname, features = '',message = message, cls = 'invisible', classy = 'invisible')
...
def stats():
    ...
    qry = "select sum(Amount_Donated) from orphanage_stats"
    mycursor.execute(qry)
    stats_donate = list(mycursor.fetchall())
    try:
        print(int(stats_donate[0][0]))
        stats_donate[0] = list(stats_donate[0])
        
    except:
        stats_donate[0] = 0

    qry = "select sum(Amount_Collected) from donation_Camp"
    mycursor.execute(qry)
    stats_collect = list(mycursor.fetchall())
    try:
        # print(int(stats_collect[0][0]))
        stats_collect[0] = list(stats_collect[0])
        
    except:
        stats_collect[0] = 0 
    print(stats_collect)

    purse = stats_collect[0][0] - stats_donate[0][0]
    
    # purse = 0

    qry1 = "select distinct(orphanage_id) from orphanage_stats;"
    mycursor.execute(qry)
    distinct_orphanages = mycursor.fetchall()
    print(distinct_orphanages, 'bhavesh')
    for i in distinct_orphanages:
        print(i, '1')
    ...
    return render_template('statistics.html', purse= purse, donated = stats_donate[0][0], collected= stats_collect[0][0])
